...
function [worldXYZout, camPosout, camRout, intrinsicsout, distortionout] = easySBA(imageUV, worldXYZ, camPos, camR, intrinsics, intrinsicsFlag, distortion, distortionFlag)
% Usage: 
%
...
%
%
% Note - 2016-05-26 fixed output of SBA optimized rotations
if nargout == 0
...
for i=1:size(camRout,1)
  camRout(i,:)=RMtoQ(QtoRM(camRout(i,:))*QtoRM(camR(i,:)));
  %camRout(i,:)=RMtoQ(QtoRM(camR(i,:))*QtoRM(camRout(i,:)));
end

% test - it seems that camPosout requires no modification.
%camPosout=camPosout-camPos;
